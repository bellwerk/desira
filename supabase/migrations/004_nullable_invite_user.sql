...
-- Update policy: users can only update their own memberships
-- NOTE: Pending invite acceptance (user_id IS NULL) is handled via supabaseAdmin
-- in the acceptInvite server action, which bypasses RLS. The invite_token acts
-- as the credential. This policy intentionally does NOT allow updating pending
-- invites via the client to prevent unauthorized invite claiming.
